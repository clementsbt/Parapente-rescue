            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-1 md:gap-4">
              {[
                ...
                <ScrollEffect key={c.title} delay={i * 0.1}>
                  <div className="p-2 md:p-6 rounded-sm border transition-all hover:shadow-lg hover:scale-105" style={{ borderColor: "#D8E8DC", background: "#fff" }}>
                    <div className="text-lg md:text-2xl mb-1 md:mb-3">{c.icon}</div>
                    <h3 className="font-semibold text-xs md:text-base mb-0 md:mb-2" style={{ color: "#111C17" }}>{c.title}</h3>
                    <p className="text-[10px] md:text-sm leading-tight" style={{ color: "#6B7C72" }}>{c.desc}</p>
                  </div>
                </ScrollEffect>
              ))}
            </div>
